...
def UpperBound(t_buf, u, theta, t, t_pre, D, np.ndarray[np.float64_t, ndim=2] P, np.ndarray[np.float64_t, ndim=2] Q,
    ...
    if c_iter == topn: # Initial setting
        ...
        for d in xrange(D):
            if weight[d, 0] < 0:
                weight[d, 0] = -1 * weight[d,0]
                tmp = lower[d, 0]
                lower[d, 0] = -1 * upper[d, 0]
                upper[d, 0] = -1 * tmp
                value[d, 0] = -1 * value[d, 0]
            B -= weight * lower[d, 0]
            util[d] = [max((value[d, 0] + theta) / weight[d,0], (value[d, 0] - theta) / weight[d,0]), weight[d,0], upper[d,0]-lower[d,0]]
